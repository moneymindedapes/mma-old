...
const standby_wallet = process.env.MMA;
const signer = process.env.RHSS;
...
async function assignMinter() {
  const setAssignMinter = {
    'Transaction Type': 'AccountSet',
    Account: standby_wallet.address,
    NFTokenMinter: signer.address,
    SetFlag: xrpl.AccountSetAsfFlags.asfAuthorizedNFTokenMinter,
  };
  const setMinterPrepared = await client.autofill(setAsignMinter);
}
//burn if need be

// async function mintToken() {
//   const client = await connect();
//   const issuerWallet = xrpl.Wallet.fromSeed(mma);
//   console.log(token);
//   await disconnect(client);
// }
/*
610 Money Minded Apes gassed the jet up and are heading to the XRP Islands to discuss the future of the crypto market.

Visit www.moneymindedapes.com for more information.
*/
async function mintToken() {
